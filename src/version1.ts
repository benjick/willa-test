 ...
 */
export function getSortedTableNames(db: Database) {
    ...
    while(sortedTables.length < db.length) {
        ...
        for (const table of db) {
            ...
            const dependenciesAvailable = foreignTables
                .every((tableName: string) => sortedTables.includes(tableName))
        ...
        // with the dependencies. Maybe a table is referring to another table which doesn't
        // exist or there are some circular dependencies. To prevent being stuck in infinite
        // loop we throw an error here.
        if (!didChange) {
            throw new Error('Nothing happened for a full loop. Maybe your schema is borked 🤷')
        }
    }
    return sortedTables
}